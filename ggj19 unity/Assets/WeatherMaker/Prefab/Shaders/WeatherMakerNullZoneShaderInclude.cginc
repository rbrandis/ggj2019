...
uniform float4 _NullZonesParams[MAX_NULL_ZONE_COUNT];
...
inline bool CheckNullZoneMask(int nullIndex)
{
	...
	uint mask = uint(_NullZonesMin[nullIndex].w); // w of min pos is mask
...
inline void ClipWorldPosNullZone(int nullIndex, float3 worldPos)
{
	...
	// as long as the world pos is outside all null zones it can render
	clip(-0.5 + (
...
inline fixed ClipWorldPosNullZoneAlpha(int nullIndex, float3 worldPos)
{
	...
	else
	{
		...
		return min(1.0, d * fade * _NullZonesParams[nullIndex].x); // w of max pos is fade
